...
public class VocalCounter {
	...
	public static boolean counter(String fileName, char vowel, String fileResultName) throws IOException {
		...
		while ((linea = br.readLine()) != null) {
			...
			// Por cada linea analizamos caracter a caracter, y si coincide sumamos al
			// contador
		...
		// Una vez terminado el analisis del archivo escribimos en el fichero de
		// resultado el contador
		pw.write(String.valueOf(cont));
		pw.flush();
		pw.close();

		// Comprobamos que el resultado se ha escrito en el fichero del resultado y
		// devulvemos true
		// en caso de que asi sea, de lo contrario devolvemos false
		br = FilePropierties.getBufferedReader(fileResultName);
		if (br.readLine().equals(String.valueOf(cont))) {
			return true;
		} else {
			return false;
		}
